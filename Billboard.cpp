...
void Billboard::CreateInstanceData()
{
...
void Billboard::AddInstanceData(vector<Vertex> datas)
{
	if (bDatas.size() == 0) return;
...
void Billboard::CopybDatas(vector<Vertex>& datas, UINT count)
{
	if (bDatas.empty() == true) return;
	...
	for (it = bDatas.end() - 1; it != (bDatas.end() - (count + 1)); it--)
	{
		datas.push_back(*it);
	}
}